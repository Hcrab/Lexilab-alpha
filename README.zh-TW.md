...
在詞源通（LexiLamp）重新設計之前，我曾把英文練習做成每日測驗平台。教師輸入字彙並發布測驗；學生先預習詞義，再填空、造句，最後回顧成績和錯題。這個儲存庫保存的就是那個階段的程式碼。
...
這是 2025 年測驗階段的作品存檔，記錄了後來 LexiLamp 的起點。部分舊頁面仍有 TypeScript 型別錯誤，因此目前建置時會略過型別檢查；自動化測試和部署防護也尚未完善。它適合在本機體驗與回顧程式碼，不建議直接部署為線上教學系統。
