...
const FILL_BLANK_POINTS = 2;
const SENTENCE_POINTS = 4;

function QuizResults() {
  const router = useRouter()
  const { id: resultId, quizId, result: resultString } = router.query;
  ...
  // Fetch result and existing bookmarks
  useEffect(() => {
      ...
      if (router.isReady) {
        ...
        if (resultString && typeof resultString === 'string') {
          ...
          try {
            ...
            if (!resultData.details || !Array.isArray(resultData.details.questions)) {
          ...
          setError("No quiz result to display.");
          setIsLoading(false);
          return;
        }

        // 2. Get Existing Bookmarks for this Quiz Attempt
        if (user && resultData) {
          try {
            // Fetch error question bookmarks
            const errorRes = await authFetch(`/api/bookmarks?username=${user.username}&type=error_question&result_id=${resultData.id}`);
            if (errorRes.ok) {
    ...
    fetchAllData();
  }, [router.isReady, resultId, resultString, user]);
  ...
  const handleBookmark = async (question: Question, questionIndex: number) => {
    ...
    if (existingBookmark) {
      ...
      // --- Create new bookmark ---
      const quizId = result.quiz_id || result.details?.quiz_id;
      if (!quizId) {
        alert("Cannot bookmark: quiz ID is missing.");
        return;
      }
      ...
      const questionPayload = {
        ...
        word: question.word || '',
        question_prompt: question.prompt || `Sentence for "${question.word}"`,
  ...
  return (
    <main className="p-4 md:p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">{result.details.name || 'Quiz'} - Review</h1>

      <div className="my-6 p-4 bg-gray-100 rounded-lg text-center">
        <h2 className="text-xl font-semibold">Final Score</h2>
        <p className="text-4xl font-bold text-blue-600">{result.score ?? 0} / {result.total_score ?? 0}</p>
      </div>
      ...
      <>
        ...
        <div className="space-y-4">
          {filteredQuestions.map((q, i) => {
            ...
            const points = q.type === 'fill-in-the-blank' ? (q.correct ? FILL_BLANK_POINTS : 0) : q.score;

            return (
              <div key={i} className={`border-l-4 p-4 rounded-md ${isCorrect ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50'}`}>
                <div className="flex justify-between items-start">
                  <p className="text-lg font-semibold text-gray-700">{q.prompt?.replace('___', `[${q.word}]`) || `Sentence for "${q.word}"`}</p>
                  <span className={`font-bold ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
                    {points !== undefined ? `${points} pts` : (isCorrect ? `${FILL_BLANK_POINTS} pts` : '0 pts')}
                  </span>
                </div>
                ...
                {!isCorrect && q.correctAnswer && (
                  <p className="mt-2">Correct answer: <span className="font-mono p-1 bg-green-200 rounded text-green-800">{q.correctAnswer}</span></p>
                )}

                {(q.type === 'fill-in-the-blank' || q.type === 'sentence') && q.feedback && (
...
export default withAuth(QuizResults)
