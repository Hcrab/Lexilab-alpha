      ...
      const fetchAllData = async () => {
        try {
          ...
          const [statsResponse, resultsResponse, progressResponse] = await Promise.all([
            authFetch(`/api/stats/users/${user.username}`),
            authFetch(`/api/results?username=${user.username}`),
            authFetch(`/api/progress/${user.username}`)
          ]);
          ...
          const pending = progressData.filter((quiz: any) => quiz.status === 'pending');
