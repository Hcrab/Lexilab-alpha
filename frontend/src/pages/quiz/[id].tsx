...
function QuizPage() {
  ...
  useEffect(() => {
        ...
        .then(data => {
          ...
          if (savedProgressJSON) {
            try {
              ...
              if (savedProgress.shuffledFillInBlanks && savedProgress.shuffledFillInBlanks.length === categorizedData.fillInTheBlanks.length) {
                console.log("Restoring progress from localStorage...");
                setShuffledFillInBlanks(savedProgress.shuffledFillInBlanks);
                setAnswers(savedProgress.answers);
                setCurrentSection(savedProgress.currentSection);
                setSectionStatus(savedProgress.sectionStatus);
                restored = true;
              } else {
                console.warn("Saved progress found but seems invalid for this quiz. Discarding.");
                localStorage.removeItem(storageKey);
              }
            } catch (e) {
              console.error("Failed to parse saved progress. Discarding.", e);
              localStorage.removeItem(storageKey);
            }
          }
    ...
    const fetchResultsForReview = (resultId: string) => {
      authFetch(`/api/results/${resultId}`)
        .then(res => {
  ...
  // --- Memoized Save Progress Function ---
  const saveProgress = useCallback(() => {
    if (!isReviewMode && id && quizData) {
      const storageKey = `quiz-progress-${id}`;
      const progressData = {
        answers,
        currentSection,
        sectionStatus,
        shuffledFillInBlanks,
        timestamp: new Date().toISOString(),
      };
      localStorage.setItem(storageKey, JSON.stringify(progressData));
    }
  }, [isReviewMode, id, quizData, answers, currentSection, sectionStatus, shuffledFillInBlanks]);
  ...
  // --- Autosave Progress Effect ---
  useEffect(() => {
    if (!isAutosaveEnabled || isReviewMode) {
      return;
    }
  ...
  }, [isAutosaveEnabled, isReviewMode, saveProgress]);
  ...
  const handleSubmit = async () => {
    ...
    const scoringPromises = questionsToScore.map(q => limit(async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error(`AI scoring failed for "${q.word}"`);
          throw new Error(`AI scoring failed for "${q.word}"`);
        }
        scoreData = await res.json();
      } catch (err) {
        alert(`An error occurred while scoring "${q.word}". It will be marked as incorrect.`);
        scoreData = q.type === 'fill-in-the-blank'
    ...
    const finalSection2Details = fillInTheBlankQuestions.map(q => {
      ...
      return {
        ...
        type: q.type,
        word: q.word,
        correct: scored ? scored.correct : (q.answer.trim() === ''), // Mark unscored as incorrect
        feedback: scored ? scored.feedback : (q.answer.trim() === '' ? 'No answer provided.' : null)
      };
    });
            ...
            {!isReviewMode && (
              <>
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="autosave-toggle" className="ml-2 text-sm text-gray-600">
                    Autosave
                  </label>
                </div>
              </>
