...
const EditQuiz: FC<EditQuizProps> = ({ quiz, items, setItems, onFinish, onCancel }) => {
      ...
      <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
        {items.map((it, i) => (
          <div key={i} className="p-3 bg-gray-50 rounded border space-y-2">
            ...
            {it.definition && <textarea rows={1} className="border w-full p-1 rounded" value={it.def} onChange={e => { const c = [...items]; c[i].def = e.target.value; setItems(c); }} placeholder="Definition" />}
            {it.blank && <textarea rows={1} className="border w-full p-1 rounded" value={it.clue} onChange={e => { const c = [...items]; c[i].clue = e.target.value; setItems(c); }} placeholder="Fill‑in‑the‑blank sentence" />}
          </div>
        ))}
      </div>
        ...
        {quizzes.map(q => (
          <tr key={q._id} className="border-b hover:bg-gray-50">
            ...
            <td className="p-3 space-x-2 whitespace-nowrap">
              ...
              <a href={`/quiz/${q._id}?by_admin=true`} target="_blank" rel="noopener noreferrer" className="px-3 py-1 bg-blue-600 text-white rounded text-sm inline-block">Take</a>
            </td>
          </tr>
        ))}
      </tbody>
...
function QuizzesPage() {
  ...
  const handleGenerateSaturdayItems = async () => {
    ...
    if (publishAt) {
      try {
        const saturdayDate = new Date(publishAt);

        const endDate = new Date(saturdayDate);
        endDate.setDate(saturdayDate.getDate() - 1);

        const startDate = new Date(saturdayDate);
        startDate.setDate(saturdayDate.getDate() - 5);

        body.source_start_date = startDate.toISOString().split('T')[0]; // YYYY-MM-DD
        body.source_end_date = endDate.toISOString().split('T')[0];   // YYYY-MM-DD

        console.log(`Generating Saturday quiz based on scheduled time: ${publishAt}`);
        console.log(`Source quiz date range: ${body.source_start_date} to ${body.source_end_date}`);

      } catch (e) {
        alert("The scheduled 'Publish At' date is invalid. Please check it.");
        setGenerating(false);
        return;
      }
    }
    ...
    try {
      ...
      if (fetchedItems.length === 0) {
        alert("Could not find enough questions from the specified date range.");
        setGenerating(false); // Stop generation if no items are found
        return;
      }
  ...
  const regenerateSentence = async (itemIndex: number) => {
    ...
    if (!item || !item.def) {
      alert("Cannot regenerate sentence without a definition.");
      return;
    }
