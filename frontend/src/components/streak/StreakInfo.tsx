...
const StreakInfo = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch streak data
        const streakRes = await authFetch(`/api/stats/streaks?username=${user.username}`);
        if (!streakRes.ok) {
        ...
        // Fetch quotes
        const quoteRes = await authFetch('/inspiring_quote.txt');
        ...
        const encouragementRes = await authFetch('/encouragement.txt');
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 my-4 w-full max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
        ...
        <div>
          <p className="text-4xl font-bold text-green-600">{streakData.percentile}%</p>
          <p className="text-gray-600">Better than other users</p>
        </div>
      </div>
